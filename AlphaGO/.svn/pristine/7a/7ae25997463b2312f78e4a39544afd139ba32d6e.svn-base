...
// 				printData(data,$('#stuAndStatus'),$('#studentAndStatus-template'));
				...
				$.each(data.studentList,function(i,v){
					...
					str+='	<td onclick=OpenWindow("detail.go?id='+v.id+'","상세보기",600,650)>'+v.name+'</td>';
					if(v.status){
						str+='	<td onclick=OpenWindow("check.go?assignCode='+assignCode+'&id='+v.id+'&name='+v.name+'","상세보기",1000,600)><button style="height:20px; line-height:0px;" class="btn btn-primary">'+v.status+'</button></td>';
					}else{
						str+='	<td onclick=OpenWindow("dm.go?id='+v.id+'","상세보기",800,600)><button style="height:20px; line-height:0px; background-color:red;" class="btn btn-primary">미제출</button></td>';
					}
					str+='</tr>';
				})
				str+='</table>';
				...
				<table id="aTable" class="table table-bordered text-center"style="vertical-align: middle;" >					
					...
					<c:forEach items="${assignList }" var="assign">	
						<tr class="assigns" style='font-size:0.85em;cursor:pointer;'>
							<td style="text-align: left;">${assign.className }</td>
							<td style="text-align: left;max-width: 100px;">${assign.title }</td>			
							<td>${fn:substring(assign.startDate,0,10) }</td>
							<td>${fn:substring(assign.endDate,0,10) }</td>
							<td class="tdClass">
							${assign.submitRate }%
