					...
					<p style="font-size: small;margin-bottom: 2px;">* 오늘의 시간표 *</p>
						<table class="table timeTable" id="secTimeTable"
							...
							<c:forEach begin="0" end="12" step="1" var="cnt">
								<tr id="${cnt+1 }">
									<th style="background-color: #038edc; color: white;font-size:x-small;">
									${cnt+1}교시(${cnt+9}:00~${cnt+10}:00)</th>
...
<script>	
	...
	function setCantUseRoom(data){
		//data : 수업 시간 리스트
		//수업 시작일 형식-> 2022-01-01
		var startDate = $('#startDate').val();
		//수업 종료일 형식 -> 2022-01-01
		...
		for(var i=0;i<data.length;i++){
			...
			var tr = $('#'+period);//테이블에 해당 교시 찾기
			var distd = tr.find('td:eq('+(yoil-1)+')');//해당 교시의 요일 찾기
				
			//교시와 요일이 맞는 td에 grey로 색 주기
