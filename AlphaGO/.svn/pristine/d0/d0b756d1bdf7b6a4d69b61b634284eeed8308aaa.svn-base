			  ...
			  if (result.isConfirmed) {
				  Swal.fire({
					  ...
					  title: '시험을 응시합니다'
						...
						if (result.isConfirmed){
						  window.open('<%=request.getContextPath()%>/exam/detail.go?from=list&examCode='+examCode,'시험지','width=1000,height=1100,left=500,top=200,location=no,status=no,scrollbars=yes');
						}
					})
			  }else{
				  Swal.fire({
